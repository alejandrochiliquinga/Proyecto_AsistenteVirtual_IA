...
main_window = Tk()
main_window.title("Asistente Virtual IA")
...
comandos = """
        ...
        - Reproduce..(cancion)
        - Busca..(Buscar informacion)
        - Abre..(algo)
        - Alarma..(hora en 24H)
        - Archivo..(nombre)
        - Colores (rojo, azul, amarillo)
        - Termina (Finalizar el asistente)    
"""

label_title = Label(main_window, text="Asistente Virtual IA", bg="#6DD5FA", fg="#2c3e50",
                    font=('Arial', 30, 'bold'))
...
juanita_photo = ImageTk.PhotoImage(Image.open("asistente-IA.jpg"))
...
def abre(rec):
    ...
    if task in sites:
        ...
        task("Lo siento, parece que aún no has agregado esa app o página web, usa los botones de agregar")
...
def archivo(rec):
    ...
    if file in files:
        ...
        talk("Lo siento, parece que aún no has agregado un archivo, usa los botones de agregar")
...
# Funcion de enviar mensajes por whatsapp
def enviar_mensaje(rec):
    ...
    if contact in contacts:
        ...
        talk("Lo siento, parece que aún no has agregado ese contacto, usa los botones de agregar")

# Funcion de cerrar programas
def cierra(rec):
    for task in programs:
...
# Funcion para iniciar el asistente virtual
def run_juanita():
    while True:
        try:
...
def open_w_apps():
    ...
    path_label = Label(window_apps, text="Ruta de la app", fg="white", bg="#434343", font=('Arial', 10, 'bold'))
    path_label.pack(pady=2)
...
def open_w_pages():
    ...
    title_label.pack(pady=3)
    name_label = Label(window_pages, text="Nombre de la página web", fg="white", bg="#434343", font=('Arial', 10, 'bold'))
    name_label.pack(pady=2)
...
def open_w_contacts():
    ...
    title_label.pack(pady=3)
    name_label = Label(window_contacts, text="Nombre del contacto", fg="white", bg="#434343", font=('Arial', 10, 'bold'))
    name_label.pack(pady=2)

    namecontact_entry = Entry(window_contacts)
    namecontact_entry.pack(pady=1)

    phone_label = Label(window_contacts, text="Número celular (Con código del país)", fg="white", bg="#434343", font=('Arial', 10, 'bold'))
